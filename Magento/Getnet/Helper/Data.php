<?php
namespace Magento\Getnet\Helper;
...
use Magento\Getnet\API\Transaction;
use Magento\Getnet\API\Getnet;
use Magento\Getnet\API\Token;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper {
	...
	)
	{
		...
		$client_id = $this->_scopeConfig->getValue('payment/magentogn/client_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
		$client_secret = $this->_scopeConfig->getValue('payment/magentogn/client_secret', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
		$this->seller_id = $this->_scopeConfig->getValue('payment/magentogn/seller_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
		$this->environment = $this->_scopeConfig->getValue('payment/magentogn/environment', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
	...
	public function getPaymentAction(){
		return $this->_scopeConfig->getValue('payment/magentogn/payment_action', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);

	}

	public function setLog($msg){
		$writer = 0;
		if($this->_scopeConfig->getValue('payment/magentogn/log', \Magento\Store\Model\ScopeInterface::SCOPE_STORE)) {
	...
	public function getCardGetnetDecrypt(){
		...
		if($this->_customerSession->isLoggedIn()) {
			$sql = "SELECT * FROM magento_card_getnet WHERE id_customer LIKE ".$this->_customerSession->getData('customer_id');
				...
				if ($sale->getStatus() == 'APPROVED') {
					...
					if($accepts_save_card == 1) {
						if (!in_array($part_card_number, $this->_verifycarcredit)) {
							$sql = "INSERT INTO magento_card_getnet (id_customer, hash,part_number) VALUES ('$customer_id','$hash','$part_card_number')";
					...
					if($historycard = $info['deletehistorycard']) {
						...
						foreach ($deletehistorycard as $item) {
							$this->_resource->query("DELETE FROM magento_card_getnet WHERE part_number = '$item';");
						}
					}
