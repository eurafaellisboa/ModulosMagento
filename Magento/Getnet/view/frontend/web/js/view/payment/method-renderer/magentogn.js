...
define(
    [
        ...
        'Magento_Getnet/js/model/credit-card-validation/credit-card-number-validator',
        'Magento_Getnet/js/model/credit-card-validation/custom',
        'mage/url',
        'mage/calendar',
        'mage/translate'
    ],
    function (
        ...
        function getCcAvailableFlags(flag) {
            ...
            var obj =  window.checkoutConfig.payment.magentogn.icons;
            return obj[img].url;
        }

        return Component.extend({
            defaults: {
                template: 'Magento_Getnet/payment/cc',
                creditCardType: '',
                creditCardExpYear: '',
                creditCardExpMonth: '',
                creditCardNumber: '',
            ...
            getCvvImageUrl: function () {
                return window.checkoutConfig.payment.magentogn.image_cvv;
            },
            ...
            getDetailsCard: function () {
                ...
                var json = window.checkoutConfig.payment.magentogn.cardmemory;
            ...
            getPublickey: function() {
                ...
                return window.checkoutConfig.payment.magentogn.publickey;
            },

            getIcons: function (type) {
                return window.checkoutConfig.payment.magentogn.icons.hasOwnProperty(type) ?
                    window.checkoutConfig.payment.magentogn.icons[type]
                    : false;
            },
            getCcAvailableTypesValues: function () {

                return _.map(window.checkoutConfig.payment.magentogn.ccavailabletypes, function (value, key) {
                    return {
                        'value': key,
                        'type': value
                    };
            ...
            getCcYearsValues: function () {
                return _.map(window.checkoutConfig.payment.magentogn.years, function (value, key) {
                    return {
                        'value': key,
                        'year': value
                    };
            ...
            getCcMonthsValues: function () {
                return _.map(window.checkoutConfig.payment.magentogn.months, function (value, key) {
                    return {
                        'value': key,
                        'month': value
                    };
            ...
            getTypeViewCreditCard: ko.computed(function () {
                return window.checkoutConfig.payment.magentogn.type_view_credit_card;
            }),
            getSaveCardctive: ko.computed(function () {
                return window.checkoutConfig.payment.magentogn.accepts_save_card;
            }),

            getCustomScript: function () {
                $('.cardmemory').on('click', function(){

                    var type_view_credit_card = window.checkoutConfig.payment.magentogn.type_view_credit_card;

                    if(type_view_credit_card == 1) {
                        $('#magentogn_fullname').val(this.getAttribute('name-card')).trigger('keyup');
                        $('#magentogn_cc_number').val(this.getAttribute('number-card')).trigger('keyup');
                        $('#magentogn_expiration').val(this.getAttribute('month-card')).trigger('change');
                        $('#magentogn_expiration_yr').val(this.getAttribute('year-card')).trigger('change');
                    }else if(type_view_credit_card == 2){
                        $('#magentogn_cc_number').val(this.getAttribute('number-card')).trigger('keyup');
                        $('#magentogn_fullname').val(this.getAttribute('name-card')).trigger('keyup');
                        $('#magentogn_mm_yy').val(this.getAttribute('month-card')+' / '+this.getAttribute('year-card')).trigger('keyup');

                        $('#magentogn_cc_cid').focus();
                ...
                $('.btndeletedhistorycard').on('click', function(){
                    var objdelhustory =  $('#magentogn_deletehistorycard');
            ...
            getInstall: function () {
                ...
                //console.log(valor);
                var type_interest 	= window.checkoutConfig.payment.magentogn.type_interest;
                var info_interest 	= window.checkoutConfig.payment.magentogn.info_interest;
                var min_installment = window.checkoutConfig.payment.magentogn.min_installment;
                var max_installment = window.checkoutConfig.payment.magentogn.max_installment;
            ...
            getData: function () {
                ...
                if(this.getTypeViewCreditCard() == 2){
                    x = $('#magentogn_mm_yy').val();
                    x = x.replace(/\s/g,'');
                    array = x.split('/');
                    cc_exp_month = array[0];
                    cc_exp_year = array[1];

                    cc_cid = $('#magentogn_cc_cid').val();
                    cc_type = this.creditCardType();
                    cc_number =  $('#magentogn_cc_number').val().replace(/\s/g,'');
