...
class Transaction
{
    ...
    public function addPayItauShopline($payment, $amount){
        ...
        $boletoitau_taxvat = $info['boletoitau_taxvat'];
        ...
        $data = array(
            'apiKey' => $this->storeConfig('payment/boletoitau/api_key'),
            ...
            'payer_cpf_cnpj' => $boletoitau_taxvat, // cpf ou cnpj
            ...
            'days_due_date' => $this->storeConfig('payment/boletoitau/days_due_date'), // dias para vencimento do boleto
            'late_payment_fine' => $this->storeConfig('payment/boletoitau/late_payment_fine'),// Percentual de multa após vencimento.
            'per_day_interest' => $this->storeConfig('payment/boletoitau/per_day_interest'), // Juros após vencimento.
            'items' => $itemOrderm,
        );
...
// Exemplo de como capturar a resposta json
            ...
            if($this->storeConfig('payment/boletoitau/log')) {
                $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/boletoitau.log');
    ...
    public function cancelBoleto($data){
        $data = array(
            'apiKey' => $this->storeConfig('payment/boletoitau/api_key'),
            'token' => $this->storeConfig('payment/boletoitau/token'),
            'status' => 'canceled',
            'transaction_id' => $data,
        );
        $data_post = json_encode( $data );
