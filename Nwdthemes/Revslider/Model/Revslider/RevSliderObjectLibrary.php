...
class RevSliderObjectLibrary extends RevSliderFunctions {
    ...
    private $customsvgpath		= '/svg/objects/';
	...
	public function __construct() {
		$this->_curl = FA::getCurlHelper();

		$this->upload_dir = FA::wp_upload_dir();

		$this->font_icon_paths = array(
			Framework::$RS_PLUGIN_URL.'public/assets/fonts/font-awesome/css/font-awesome.min.css',
			Framework::$RS_PLUGIN_URL.'public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.min.css'
		);
		$this->font_icon_paths = FA::apply_filters('revslider_object_library_icon_paths', $this->font_icon_paths);
	}
	...
	public function _get_list($force = false){
		...
		$last_check	= FA::get_option('revslider-library-check');

		if($last_check == false){ //first time called
			$last_check = 1296001;
			FA::update_option('revslider-library-check', time());
		}
		...
		if(time() - $last_check > 1296000 || $force == true){ //30 days
			...
			$code = ($validated == 'false') ? '' : FA::get_option('revslider-code', '');
			$hash = FA::get_option('revslider-library-hash', '');
			$rattr = array(
			...
			if(!FA::is_wp_error($request)){
				if($response = FA::maybe_unserialize($request['body'])){
					if('actual' != $response){
						...
						if(is_array($library)){
							if(isset($library['hash'])) FA::update_option('revslider-library-hash', $library['hash']);
							FA::update_option('rs-library', $library, false);
						}
					}
				}
			}
	...
	public function get_object_handle_by_id($id){
		$handle	 = '';
		$full	 = FA::get_option('rs-library', array());
	...
	public function _get_object_thumb($object_handle, $type, $download = false){
		...
		if($_download && $download === true && FA::wp_mkdir_p($this->upload_dir['basedir'].$path)){
			$curl = $this->_curl;
			$file = $this->upload_dir['basedir'] . $path . $object_handle;
			...
			if(!is_file($file)){
				...
				if($curl !== false){
					if($validated == 'false' && !in_array($type, $this->allowed_types, true)){
						...
						if(!FA::is_wp_error($image_data) && isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200'){
	...
	public function _get_object_layers($object_id){
		...
		if($curl !== false){
            ...
            if($validated == 'false'){
				...
				if(!FA::is_wp_error($layers_data) && isset($layers_data['body']) && isset($layers_data['response']) && isset($layers_data['response']['code']) && $layers_data['response']['code'] == '200'){
	...
	public function load_objects_with_svg(){
		...
		$online	= FA::get_option('rs-library', array());
		...
		if(file_exists($image_path) && $imgsize = getimagesize($image_path)) {
			...
			foreach($this->sizes as $size){
                ...
                if( ! file_exists($targetPath) || $force) {
					...
					FA::getImagesHelper()->image_resize_to($image_path, $width, $height, $targetPath);
                }
			}
	...
	public function get_favorites(){
		return FA::get_option('rs_obj_favorites', array());
	}
	...
	public function save_favorites($favourites){
		FA::update_option('rs_obj_favorites', $favourites);
	}
	...
	public function get_svg_sets_url(){
		...
		return FA::apply_filters('revslider_get_svg_sets', $svg_sets);
	}
	...
	public function get_svg_sets_full(){
		...
		return FA::apply_filters('revslider_get_svg_sets_full', $svg);
	}
    ...
    public function get_custom_svgs(){
        $favorite = RevSliderGlobals::instance()->get('RevSliderFavorite');;
        $library  = FA::get_option('rs-custom-library', array());
        $svgcustom = array();
        ...
        return FA::apply_filters('revslider_get_custom_svgs', $svgcustom);
    }
	...
	public function get_font_icons(){
		...
		return FA::apply_filters('revslider_get_font_icons', $font_icons);
	}
	...
	public function get_font_tags(){
		...
		return FA::apply_filters('revslider_get_font_tags', $tags);
	}
	...
	public function get_custom_tags(){
		return FA::get_option('rs-custom-library-tags', array());
	}
	...
	public function create_custom_tag($name, $type){
		...
		$tags = FA::get_option('rs-custom-library-tags', array());
		...
		$tags[$type][$lid] = $name;
		FA::update_option('rs-custom-library-tags', $tags);
	...
	public function upload_custom_item($data){
		...
		$return = FA::apply_filters('revslider_upload_custom_library_item', $return, $data);
	...
	public function edit_custom_item($id, $type, $name, $tags){
		...
		if(isset($library[$type]) && isset($library[$type]['items']) && !empty($library[$type]['items'])){
			foreach($library[$type]['items'] as $lk => $lv){
				if(strval($this->get_val($lv, 'id', 0)) === strval($id)){
					...
					FA::update_option('rs-custom-library', $library);
		...
		$return = FA::apply_filters('revslider_edit_custom_library_item', $return, $id, $type, $name, $tags);
	...
	public function delete_custom_item($id, $type){
		...
		$return = FA::apply_filters('revslider_delete_custom_library_item', $return, $id, $type);
	...
	private function import_custom_svg_file($data, $customs){
		...
		switch($info){
			...
			case 'application/zip':
				$zip = true;
				$this->download_path = Framework::$RS_PLUGIN_PATH.'rstempsvg/';
				$this->svg_remove_path = $this->download_path;
				$wp_filesystem->delete($this->download_path, true);
				$file	= FA::unzip_file($path, $this->download_path);
