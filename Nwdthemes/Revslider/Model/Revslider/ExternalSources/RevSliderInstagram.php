...
class RevSliderInstagram extends RevSliderFunctions {
	...
	*/
    ...
    public function do_init()
    {
        ...
        $connectwith = Data::$_GET[self::QUERY_CONNECTWITH];
        $id = Data::$_GET['id'];
	...
	protected function _refresh_token($token){
        ...
        if($this->transient_token_sec > 0 && false !== ($data = FA::get_transient($transient_token_name))){
            return;
        }
        ...
        FA::set_transient($transient_token_name, $token, $this->transient_token_sec);
    }
    ...
    public function get_public_photos($slider_id, $token, $count, $orig_image = ''){
        ...
        if($this->transient_sec > 0 && false !== ($data = FA::get_transient($transient_name))){
            ...
            FA::delete_transient($transient_name);
        }
	...
	function input($name, $default = null){
		return isset(Data::$_REQUEST[$name]) ? Data::$_REQUEST[$name] : $default;
	}
	...
	public function get_tag_photos($search_user_id,$count,$orig_image){
		if(!empty($search_user_id)){
			...
			if($this->transient_sec > 0 && false !== ($data = FA::get_transient( $transient_name))){
				...
				$rsp = json_decode(FA::wp_remote_fopen($url));
	...
	private function instagram_output_array_places($photos,$count,$search_user_id,$orig_image=""){
		foreach ($photos as $photo) {
			if($count > 0){
				...
				$stream['date_modified'] = FA::date_i18n( FA::get_option( 'date_format' ), ( $photo->date ) ) ;
