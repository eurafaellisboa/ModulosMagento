	...
	*/
	public function get_photo_sets($user_id, $item_count, $current_photoset){ //item count default is 10
		...
		foreach($photo_sets_list->photosets->photoset as $photo_set){
			...
			$return[] = '<option title="'.$photo_set->description->_content.'" '.FA::selected($photo_set->id , $current_photoset , false).' value="'.$photo_set->id.'">'.$photo_set->title->_content.' ('.$photo_set->photos.' photos)</option>"';
		}
		return $return;
	}
