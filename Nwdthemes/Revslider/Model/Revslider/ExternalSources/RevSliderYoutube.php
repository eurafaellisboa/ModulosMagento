...
class RevSliderYoutube extends RevSliderFunctions {
	...
	*/
	...
	public function show_playlist_videos($playlist_id, $count = 50){
		...
		FA::set_transient($transient_name, $rsp->items, $this->transient_sec);
	...
	public function get_playlist_options($current_playlist){
		...
		if(!empty($playlists)){
			foreach($playlists as $playlist){
				$return[] = '<option title="'.$playlist->snippet->description.'" '.FA::selected($playlist->id , $current_playlist , false).' value="'.$playlist->id.'">'.$playlist->snippet->title.'</option>"';
			}
		}
		return $return;
	}
