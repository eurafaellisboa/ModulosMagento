...
class RevSliderData {
    ...
    public function delete_wp_cache($method, $args = array())
    {
        ...
        $cache_key = $this->get_wp_cache_key($method, $args);
        FA::wp_cache_delete($cache_key, self::CACHE_GROUP);
    }
	...
	public function get_animations_v5($pre = ''){
        ...
        foreach(Framework::$revslider_animations as $value){
	...
	public function fill_animations(){
        if(empty(Framework::$revslider_animations)){
            ...
            Framework::$revslider_animations = (!empty($result)) ? $result : array();

            if(!empty(Framework::$revslider_animations)){
                foreach(Framework::$revslider_animations as $ak => $av){
                    Framework::$revslider_animations[$ak]['params'] = json_decode(str_replace("'", '"', $av['params']), true);
                }
			}

            if(!empty(Framework::$revslider_animations)){
                array_walk_recursive(Framework::$revslider_animations, array($this, 'force_to_boolean'));
			}
		}
	}
	...
	public function get_all_image_sizes($type = 'gallery'){
		...
		switch($type){
			...
			case 'gallery':
			default:
				$added_image_sizes = FA::get_intermediate_image_sizes();
    ...
    protected function _get_base_transitions($raw = false){
        ...
        $transitions = FA::apply_filters('revslider_data_get_base_transitions', $transitions);
