...
class RevSliderSlide extends RevSliderFunctions {
	...
	public function get_settings(){
		return FA::apply_filters('revslider_slide_get_settings', FA::apply_filters('revslider_slide_getSettings', $this->settings, $this), $this);
	}
	...
	public function get_params(){
		return FA::apply_filters('revslider_slide_get_params', FA::apply_filters('revslider_slide_getParams', $this->params, $this), $this);
	}
	...
	public function get_layers(){
		return FA::apply_filters('revslider_get_layers', FA::apply_filters('revslider_getLayers', $this->layers, $this), $this);
	}
	...
	public function init_by_slide(RevSliderSlide $slide){
		$slide = FA::apply_filters('revslider_slide_initBySlide', $slide, $this);
	...
	public function save_slide($slide_id, $data, $slider_id){
		...
		FA::do_action('revslider_slide_updateSlideFromData_post', false, $data, $this);
	...
	public function delete_slide_by_id($slide_id){
		...
		FA::do_action('revslider_slide_deleteSlide', $slide_id);
	...
	public function init_by_post_data($data, RevSliderSlide $template, $slider_id){
        ...
        $template_id = FA::get_post_meta($post_id, 'slide_template', true);
        ...
        $this->post_data = FA::apply_filters('revslider_slide_initByPostData', $data, $template, $slider_id, $this);
		...
		if($this->get_val($this->params, array('seo', 'set'), false) == true && $this->get_val($this->params, array('seo', 'type'), 'regular') == 'regular'){
			...
			if(!empty($matches)){
				foreach($matches as $match){
					...
					$meta_val = FA::get_post_meta($post_id, $meta, true);
        ...
        if(!in_array($this->get_val($this->params, array('bg', 'type'), 'trans'), array('trans', 'solid'), true)){
            if($this->get_val($this->params, array('bg', 'imageFromStream'), false) === true){ //if image is choosen, use featured image as background // && $this->get_val($this->params, array('bg', 'type')) == 'image'
                $tid = FA::get_post_thumbnail_id($post_id);

                if(!empty($tid)){
                    $this->set_image_by_image_id($tid);

                    //set the thumbnail image
                    $thumbnail_url = FA::wp_get_attachment_image_src($tid, 'thumbnail');
	...
	private function set_layers_by_post($post, $slider_id){
		...
		$post = FA::apply_filters('revslider_slide_setLayersByPostData_pre', $post, $slider_id, $this);
        ...
        $ptid       = FA::get_post_thumbnail_id($post_id);
		...
		$attr		= array(
			...
			'description'		=> $this->get_text_intro($this->get_val($post, 'description'), (int)$excerpt_limit),
		...
		foreach($img_sizes as $img_handle => $img_name){
			$featured_image_url = FA::getImagesHelper()->wp_get_attachment_image_src($ptid, $img_handle);
		...
		$attr = FA::apply_filters('revslider_slide_setLayersByPostData_post', $attr, $post, $slider_id, $this);

		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){
				$text = $this->get_val($layer, 'text');
				$text = FA::apply_filters('revslider_mod_meta', $text, $post['ID'], $post); //option to add your own filter here to modify meta to your likings
				...
				if($this->get_val($layer, 'type', 'text') === 'image' && $this->get_val($layer, array('media', 'imageFromStream'), false) === true){
					$featured_image_url = FA::getImagesHelper()->wp_get_attachment_image_src($ptid, 'full');
	...
	public function get_excerpt_by_id($id, $limit = 55, $type = 'words'){
		...
		return FA::apply_filters('revslider_getExcerptById', $excerpt, $post, $limit);
	}
	...
	public function set_post_data($text, $attr, $post_id){
		...
		$text = FA::apply_filters('rev_slider_insert_meta', $text, $post_id);
		...
		if(!empty($arrMatches)){
			foreach($arrMatches as $matched){
				foreach($matched as $match){
					...
					$meta = str_replace('%', '',$meta);
					$meta = str_replace('_REVSLIDER_', '-', $meta);
					$metaValue = FA::get_post_meta($post_id, $meta, true);
		...
		if(!empty($arrMatches)){
			foreach($arrMatches as $matched){
				foreach($matched as $match) {
					...
					$avatar = FA::get_avatar_url($this->get_val($attr, 'authorID'), array('size'=> $vals[0]));
	...
	public function init_by_stream_data($data, $template, $slider_id, $sourcetype, $additions){
		$a = FA::apply_filters('revslider_slide_initByStreamData', array('post_data' => $data, 'template' => $template, 'slider_id' => $slider_id, 'sourcetype' => $sourcetype, 'additions' => $additions), $this);
		...
		switch($a['sourcetype']){
			...
			break;
			default:
				$return = FA::apply_filters('revslider_slide_initByStreamData_sourceType', false, $a, $this);
	...
	private function init_by_facebook($slider_id, $additions){
		$this->post_data = FA::apply_filters('revslider_slide_initByFacebook_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if($this->get_val($this->params, array('bg', 'type')) == 'image'){ //if image is choosen, use featured image as background
			...
			$this->image_url = (FA::is_ssl()) ? str_replace('http://', 'https://', $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = FA::apply_filters('revslider_slide_initByFacebook_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_twitter($slider_id, $additions){
		$this->post_data = FA::apply_filters('revslider_slide_initByTwitter_pre', $this->post_data, $slider_id, $additions, $this);
        ...
        if(in_array($this->get_val($this->params, array('bg', 'type')), array('html5', 'trans', 'image', 'streamtwitter', 'streamtwitterboth'), true)){ //if image is choosen, use featured image as background
			...
			if($img == ''){
				...
				if(is_array($image_url_array) && isset($image_url_array[0])){
					$img = (FA::is_ssl()) ? $this->get_val($image_url_array[0], 'media_url_https') : $this->get_val($image_url_array[0], 'media_url');
				}
			}
		...
		$this->post_data = FA::apply_filters('revslider_slide_initByTwitter_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_instagram($slider_id, $additions = array()){
		$this->post_data = FA::apply_filters('revslider_slide_initByInstagram_pre', $this->post_data, $slider_id, $this);
		...
		$this->post_data = FA::apply_filters('revslider_slide_initByInstagram_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_flickr($slider_id, $additions){
		$this->post_data = FA::apply_filters('revslider_slide_initByFlickr_pre', $this->post_data, $slider_id, $this);
		$this->id		 = $this->get_val($this->post_data, 'id');
		$this->set_param('title', $this->get_val($this->post_data, 'title'));
		$this->set_param(array('publish', 'state'), 'published');
		...
		$this->post_data = FA::apply_filters('revslider_slide_initByFlickr_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_youtube($slider_id, $additions){
		$this->post_data = FA::apply_filters('revslider_slide_initByYoutube_pre', $this->post_data, $slider_id, $additions, $this);
		...
		$this->post_data = FA::apply_filters('revslider_slide_initByYoutube_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_vimeo($slider_id, $additions){
		$this->post_data = FA::apply_filters('revslider_slide_initByVimeo_pre', $this->post_data, $slider_id, $additions, $this);
		...
		$this->post_data = FA::apply_filters('revslider_slide_initByVimeo_post', $this->post_data, $slider_id, $additions, $this);
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false){
		...
		$text = FA::apply_filters('revslider_slide_set_stream_data_pre', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
		...
		return FA::apply_filters('revslider_slide_set_stream_data_post', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
	}
	...
	private function set_layers_by_stream($slider_id, $stream_type, $additions = array()){
		$a				= FA::apply_filters('revslider_slide_setLayersByStreamData_pre', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
		...
		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){
				$text = $this->get_val($layer, 'text');
				$text = FA::apply_filters('revslider_mod_stream_meta', $text, $slider_id, $stream_type, $this->post_data); //option to add your own filter here to modify meta to your likings
		...
		$a = FA::apply_filters('revslider_slide_setLayersByStreamData_post', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
	...
	public function return_stream_data($stream_type, $additions = array()){
		$img_sizes = $this->get_all_image_sizes($stream_type);
		$attr = array();
		$attr = FA::apply_filters('revslider_slide_return_stream_data_pre', $attr, $stream_type, $additions, $img_sizes);
		...
		switch($stream_type){
			...
			case 'instagram':
				...
				$attr1 = array(
					...
					'date'		=> FA::date_i18n(FA::get_option('date_format').' '.FA::get_option('time_format'), $this->get_val($this->post_data, 'taken_at_timestamp', false)),
					'author_name' => $this->get_val($additions, 'instagram_user'),
				);
		...
		return FA::apply_filters('revslider_slide_return_stream_data_post', $attr, $stream_type, $additions, $img_sizes);
	}
	...
	public function save_layers(){
		...
		$this->layers	= FA::apply_filters('revslider_slide_saveLayers', $this->layers, $this->static_slide, $this);
	...
	public function save_params(){
		...
		$table = ($this->static_slide) ? $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES : $wpdb->prefix . RevSliderFront::TABLE_SLIDES;
		$this->params = FA::apply_filters('revslider_slide_saveParams', $this->params, $this->static_slide, $this);
	...
	public function save_settings(){
		...
		$table = ($this->static_slide) ? $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES : $wpdb->prefix . RevSliderFront::TABLE_SLIDES;
		$this->settings = FA::apply_filters('revslider_slide_save_settings', $this->settings, $this->static_slide, $this);
	...
	public function get_children(){
		...
		return FA::apply_filters('revslider_slide_getArrChildren', $this->children, $this);
	}
	...
	public function create_slide($slider_id, $obj = '', $static = false, $id = false){
		...
		$insert	= FA::apply_filters('revslider_slide_createSlide', $insert, $slider_id, $static, $this);
	...
	public function init_by_data($slide){
		$slide = FA::apply_filters('revslider_slide_init_by_data', FA::apply_filters('revslider_slide_initByData', $slide));
		...
		$image_url			= (FA::is_ssl()) ? str_replace('http://', 'https://', $image_url) : $image_url;
    ...
    public function get_slide_ids_by_slider_id($slider_ids){
        ...
        if(is_array($slider_ids) && !empty($slider_ids)){
            ...
            $slides_data = $wpdb->get_results($wpdb->prepare("SELECT `id`, `slider_id`, `slide_order` FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDES." WHERE slider_id IN(".$in.") ORDER BY slider_id,slide_order ASC", $slider_ids), Query::ARRAY_A);
        }else{
            $slides_data = $wpdb->get_results($wpdb->prepare("SELECT `id`, `slider_id`, `slide_order` FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDES." WHERE slider_id = %d ORDER BY slide_order ASC", $slider_ids), Query::ARRAY_A);
        }
    ...
    public function get_slides_by_slider_id($slider_id, $published = false, $wmpl = false, $first = false, $init_layer = true, $fetch_single = false){
        ...
        $slides_data = FA::wp_cache_get($cache_key, self::CACHE_GROUP);
        if (!$slides_data) {
            $slides_data = $wpdb->get_query_results($slides_data_sql, Query::ARRAY_A);
            FA::wp_cache_set($cache_key, $slides_data, self::CACHE_GROUP);
        }
		...
		foreach($slides_data as $slide_data){
			...
			if ($published) {
				...
				if ($storePermissions && ! in_array(FA::getStoreId(), $storeIds)) {
					continue;
				}
			}
	...
	public function get_params_for_export(){
		...
		return FA::apply_filters('revslider_slide_getParamsForExport', FA::apply_filters('revslider_slide_getParamsForExport', $params, $this), $this);
	}
	...
	public function get_layers_for_export(){
		...
		return FA::apply_filters('revslider_slide_get_layers_for_export', FA::apply_filters('revslider_slide_getLayersForExport', $layers, $this), $this);
	}
	...
	public function get_image_attributes($layouttype){
		...
		return FA::apply_filters('revslider_slide_get_image_attributes', array(
			'type'	=> $class,
			'src'	=> $thumb,
			'style'	=> $style
		), $this);
	...
	public function get_used_fonts($full = false){
		...
		return FA::apply_filters('revslider_slide_getUsedFonts', $fonts, $this);
	}
	...
	private function set_image_by_image_id($id){
		$id			= FA::apply_filters('revslider_slide_setImageByImageID', $id, $this);
	...
	public function set_image_by_id($imageID, $size = 'full'){
		$a = FA::apply_filters('revslider_slide_setImageByID', array('imageID' => $imageID, 'size' => $size), $this);
	...
	public function get_categories_html($cat_ids, $tax = null, $post_id = ''){
		...
		if(!empty($errors)){
			...
			$this->throw_error(__('retrieving categories error: '.FA::esc_html($err)));
		}
		...
		foreach($categories as $category){
			...
			$link = FA::get_category_link($category['term_id']);
			$name = $category['name'];
			$list.= ($sep == true) ? ',' : '';
			$list.= (!empty($link)) ? '<a href="' . FA::esc_url($link) . '" title="' . FA::esc_attr(sprintf(__('View all posts in %s', 'revslider'), $category['name'])) .'" '. $rel .'>'. $name .'</a>' : $name;
			$sep  = true;
		}
