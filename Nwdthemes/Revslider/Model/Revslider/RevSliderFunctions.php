...
class RevSliderFunctions extends RevSliderData {
    ...
    protected function _get_global_settings(){
		$gs = FA::get_option('revslider-global-settings', '');
		if(!is_array($gs)){
			$gs = json_decode($gs, true);
		}

		return FA::apply_filters('rs_get_global_settings', $gs);
	}
	...
	public function set_global_settings($global){
		...
		return FA::update_option('revslider-global-settings', $global);
	}
	...
	public function get_addition($key = ''){
		$additions = (array)FA::get_option('revslider-additions', array());
	...
	public function get_get_var($key, $default = ''){
        return $this->get_var(Data::$_GET, $key, $default);
    }
	...
	public function get_request_var($key, $default = ''){
        return (array_key_exists($key, $_POST)) ? $this->get_var($_POST, $key, $default) : $this->get_var(Data::$_GET, $key, $default);
    }
	...
	public function get_url_attachment_image($id, $size = 'full'){
		$image	= FA::getImagesHelper()->wp_get_attachment_image_src($id, $size);
		$url	= (empty($image)) ? false : $this->get_val($image, 0);
		if($url === false){
			$url = FA::wp_get_attachment_url($id);
		}
	...
	public function get_image_id_by_url($image_url){
		return FA::getImagesHelper()->attachment_url_to_postid($image_url);
	}
	...
	public function get_image_id_by_basename($basename){
		return FA::getImagesHelper()->get_image_id_by_url($basename);
	}
	...
	public static function get_base_url(){
		if(FA::is_multisite() == false){ //without multisite
			$url = FA::content_url().'/';
		}else{	//for multisite
	...
	public static function esc_attr_deep($value){
		$value = is_array($value) ? array_map(array('\Nwdthemes\Revslider\Model\Revslider\RevSliderFunctions', 'esc_attr_deep'), $value) : FA::esc_attr($value);
	...
	public function get_post_type_assoc($put_to_top = array()){
		...
		$custom_types	= FA::get_post_types(array('_builtin' => false));
	...
	public static function get_post_type_title($post_type){
		$obj_type	= FA::get_post_type_object($post_type);
	...
	public function get_post_type_taxonomies($post_type){
		$names	= array();
		$tax	= FA::get_object_taxonomies(array('post_type' => $post_type), 'objects');
	...
	public function import_media($file_url, $folder_name){
		...
		if(@fclose(@fopen($file_url, 'r'))){ //make sure the file actually exists
			...
			if($_atc_id !== $atc_id && $atc_id !== false && $atc_id !== NULL){ //&& $_atc_id !== false && $_atc_id !== NULL
				...
				$_save_dir = FA::getImagesHelper()->get_attached_file($atc_id);
			...
			if($atc_id == false || $atc_id == NULL){
				...
				$artdata = array( //create an array of attachment data to insert into wp_posts table
					'post_author'	 => 1,
					'post_date'		 => FA::current_time('mysql'),
					'post_date_gmt'	 => FA::current_time('mysql'),
					'post_title'	 => $filename,
					'post_status'	 => 'inherit',
					'comment_status' => 'closed',
					'ping_status'	 => 'closed',
					'post_name'		 => FA::sanitize_title_with_dashes(str_replace('_', '-', $filename)),
					'post_modified'	 => FA::current_time('mysql'),
					'post_modified_gmt' => FA::current_time('mysql'),
					'post_parent'	 => '',
				...
				$attach_id = FA::wp_insert_attachment($artdata, $s_dir);

				//generate metadata and thumbnails
				FA::add_filter('intermediate_image_sizes_advanced', array('\Nwdthemes\Revslider\Model\Revslider\RevSliderFunctions', 'temporary_remove_sizes'), 10, 2);
			...
			$return	 = array('id' => $attach_id, 'path' => FA::getImagesHelper()->imageFile($s_dir));
		}
	...
	public function get_static_css(){
		return FA::get_option('revslider-static-css', '');
	}
	...
	public function update_static_css($css){
		...
		FA::update_option('revslider-static-css', $css);
	...
	public function print_clean_font_import(){
		...
		if(!empty(Framework::$revslider_fonts['queue'])){
			foreach(Framework::$revslider_fonts['queue'] as $f_n => $f_s){
				...
				$ret .= '<link href="'.FA::esc_html($f_s['url']).'" rel="stylesheet" property="stylesheet" media="all" type="text/css" >'."\n";
			}
		}
						...
						if(!empty($body)){
							foreach($body as $b){
								if(preg_match($regex_url, $b, $found_fonts)){
									...
									$f_c = FA::wp_remote_get($found_font);
...
"@font-face {
  ...
  src: local('".$f_family."'), local('".$f_family."'), url(".$base_url.'/gfonts/'. $font_name . '/' . $font_name . '-' . $weight . '.woff2'.") format('woff2');
		...
		return FA::apply_filters('revslider_printCleanFontImport', $ret);
	}
	...
	public function convert_post_date($date, $with_time = false){
		if(!empty($date)){
			$date = ($with_time) ? FA::date_i18n(FA::get_option('date_format').' '.FA::get_option('time_format'), strtotime($date)) : FA::date_i18n(FA::get_option('date_format'), strtotime($date));
		}
		return $date;
	}
	...
	public function delete_google_fonts(){
		FA::update_option('rs_google_font', time());
	}
	...
	public function _is_gutenberg_page(){
		if(isset(Data::$_GET['action']) && Data::$_GET['action'] == 'elementor') return false; // Elementor Page Edit
		if(isset(Data::$_GET['vc_action']) && Data::$_GET['vc_action'] == 'vc_inline') return false; // WP Bakery Front Edit
		if(function_exists('is_gutenberg_page') && is_gutenberg_page()) return true; // Gutenberg Edit with WP < 5
		$current_screen = FA::get_current_screen();
	...
	public function get_custom_slidetransitions(){
		$custom = FA::get_option('revslider_template_slidetransitions', array());

		return FA::apply_filters('rs_get_custom_slidetransitions', $custom);
	}
	...
	public function save_custom_slidetransitions($template){
		...
		return (FA::update_option('revslider_template_slidetransitions', $custom)) ? $new_id : false;
	}
	...
	public function delete_custom_slidetransitions($template){
		...
		if($id > 0 && isset($custom[$id])){
			...
			if(FA::update_option('revslider_template_slidetransitions', $custom)) return true;
		}
