...
class RevSliderSlider extends RevSliderFunctions {
	...
	public function init_by_id($sid, $show_error = true){
		...
		if(empty($slider_data) && !FA::is_admin() && $show_error === true){
	...
	public function init_by_alias($alias, $show_error = true){
		...
		if(empty($slider_data) && !FA::is_admin() && $show_error === true){
			throw new \Exception('Slider with alias '.FA::sanitize_text_field(FA::esc_attr($alias)).' not found.');
		}
	...
	public function init_by_data($data){
		$data = FA::apply_filters('revslider_slider_init_by_data', $data);
		...
		$do_action = (FA::is_admin())		? false : true;
		$do_action = (FA::wp_doing_ajax())	? true : $do_action;
		$do_action = (Framework::$rs_preview_mode)	? true : $do_action;

		if($do_action){
			if(Framework::$rs_do_init_action === true){
				FA::do_action('revslider_slider_init_by_data_post', $this);
			}
		}
	...
	public function get_slider_count(){
		$wpdb = FA::getQueryHelper();
        return count($wpdb->get_results("SELECT COUNT(*) FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE `type` = '' OR `type` IS NULL", Query::ARRAY_A));
    }
    ...
    public function get_slider_by_param_string($string, $templates = false){
        ...
        return $wpdb->get_results($sql, Query::ARRAY_A);
    }
				...
				if($image != ''){
					...
					switch($altOption){
						case 'media_library';
							...
							if($id > 0) $alt = FA::get_post_meta($id, '_wp_attachment_image_alt', true);
						break;
						case 'file_name';
							$alt = $image;
						break;
					...
					switch($titleOption){
						case 'media_library';
							$id = attachment_url_to_postid($image);
							if($id > 0) $title = FA::get_the_title($id);
						break;
						case 'file_name';
							$title = $image;
						break;
    ...
    public function delete_all_slides(){
		...
		FA::do_action('revslider_slider_delete_all_slides', $this->id);
		FA::do_action('revslider_slider_deleteAllSlides', $this->id);
	}
	...
	public function delete_static_slide(){
		...
		FA::do_action('revslider_slider_delete_static_slide', $this->id);
	}
	...
	public function duplicate_slider_by_id($id, $is_template = false){
		...
		if($is_template){
			...
			$talias	= strtolower(FA::sanitize_title_with_dashes($title));
		}else{
		...
		while($this->alias_exists($talias)){ //set a new alias and title if its existing in database
			$talias = strtolower(FA::sanitize_title_with_dashes($title)) .  '-' . $ti;
			$ti++;
		}
	...
	public function update_title($new_title){
		...
		$new_title = stripslashes(FA::esc_html($new_title));
		if(!empty($new_title)){
	...
	private function duplicate_slider($title = false, $prefix = false){
		...
		if($title === false){
			...
			$new_title = ($prefix !== false) ? FA::sanitize_text_field($title.' '.$this->get_val($params, 'title')) : FA::sanitize_text_field($title);
			$new_alias = ($prefix !== false) ? FA::sanitize_title($title.' '.$this->get_val($params, 'title')) : FA::sanitize_title($title);
		...
		FA::do_action('revslider_duplicate_slider', $slider_last_id, $old_slider_id, $slides, $this);
	...
	public function save_slider($slider_id, $data){
		...
		$title	= FA::sanitize_text_field($this->get_val($params, 'title'));
		$alias	= FA::sanitize_text_field($this->get_val($params, 'alias'));
		...
		if(!FA::current_user_can('administrator') && FA::apply_filters('revslider_restrict_role', true)){
		...
		$slider_data = FA::apply_filters('revslider_slider_saveData', $slider_data);
	...
	public function get_slides($published = false, $allwpml = false, $first = false){
        ...
        $this->slides = FA::wp_cache_get($cache_key, self::CACHE_GROUP);

        if (!$this->slides) {
            $slide            = new RevSliderSlide();
            $this->slides    = $slide->get_slides_by_slider_id($this->id, $published, $allwpml, $first, $this->init_layer);
            FA::wp_cache_set($cache_key, $this->slides, self::CACHE_GROUP);
        }
	...
	public function get_slides_for_export(){
		...
		return FA::apply_filters('revslider_get_slides_for_export', FA::apply_filters('revslider_getSlidesForExport', $export));
	}
	...
	public function get_static_slide_for_export(){
		...
		return FA::apply_filters('revslider_getStaticSlideForExport', $static_slide);
	}
	...
	public function get_parent_slides($published, $gal_ids, $lang){
		...
		FA::apply_filters('revslider_get_parent_slides_pre', $lang, $published, $gal_ids, $this);
		...
		FA::apply_filters('revslider_get_parent_slides_post', $parent_slides, $published, $gal_ids, $this);
	...
	public function get_used_fonts($full = false){
		...
		return FA::apply_filters('revslider_getUsedFonts', $gf);
	}
		...
		switch($source){
			...
			case 'post':
				...
				if($subtype === 'current_post'){
					$currentProductId = FA::getCurrentProductId();
	...
	public function get_related_posts(){
		...
		$get_relateds		= FA::apply_filters('revslider_get_related_posts', $query, $post_id);
	...
	public function get_slides_from_stream($published = false){
		...
		$posts = FA::apply_filters('revslider_pre_mod_stream_data', $posts, $sourcetype, $this->id);
		...
		$posts = FA::apply_filters('revslider_post_mod_stream_data', $posts, $sourcetype, $this->id);
	...
	private function get_posts_by_categories($published = false){
		...
		return FA::apply_filters('revslider_get_posts_by_categories', $post, $this);
	}
    ...
    public function get_overview_data($slider = false, $slides = false, $slide_ids = false){
		...
		return array(
			...
			'slide_id'	=> $id,
			'slide_ids'	=> $slide_ids,
			'title'		=> FA::esc_html($slider->get_title()),
			'alias'		=> $slider->get_alias(),
			'source'	=> FA::esc_html($type),
	...
	public function get_specific_posts($gal_ids = array()){
		...
		if(!empty($gal_ids) && $gal_ids[0] !== ''){
			$posts	= $gal_ids;
			$posts	= FA::apply_filters('revslider_set_posts_list_gal', $posts, $this->get_id());
			$is_gal	= true;
		}else{
			if(isset($gal_ids[0])){
				unset($gal_ids[0]);
			...
			$posts = FA::apply_filters('revslider_set_posts_list', $posts, $this->get_id());
		}
	...
	public function get_posts_by_id($ids, $slider_id, $is_gal, $additional = array()){
		...
		$query	= FA::apply_filters('revslider_get_posts', $query, $slider_id);

		$posts= FA::getProductsHelper()->getProductsByQuery($query);
	...
	public function get_posts_by_category($slider_id, $cat_id, $sort_by = 'ID', $direction = 'DESC', $max_posts = -1, $post_types = 'any', $taxonomies = 'category', $addition = array(), $type = ''){
		$a = FA::apply_filters('revslider_get_posts_by_category', array('slider_id' => $slider_id, 'cat_id' => $cat_id, 'sort_by' => $sort_by, 'direction' => $direction, 'max_posts' => $max_posts, 'post_types' => $post_types, 'taxonomies' => $taxonomies, 'addition' => $addition, 'type' => $type), $this);
	...
	public function get_post_categories($post_id, $tax){
		...
		$cats = FA::wp_get_post_terms($post_id, $tax);
    ...
    public function modify_by_global_settings(){
        if(FA::is_admin() && !Framework::$rs_preview_mode) return true;
