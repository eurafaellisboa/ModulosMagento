...
class RevSliderTemplate extends RevSliderFunctions {
	...
	public function _download_template($uid){
		...
		$uid	= FA::esc_attr($uid);
		$code	= (FA::get_option('revslider-valid', 'false') == 'false') ? '' : FA::get_option('revslider-code', '');

		$upload_dir = FA::wp_upload_dir(); // Set upload folder
		// Check folder permission and define file location
		if(FA::getFilesystemHelper()->wp_mkdir_p($upload_dir['basedir'].$this->templates_path)){ //check here to not flood the server
			$data = array(
				'code'		=> urlencode($code),
	...
	public function _delete_template($uid){
		...
		// Check folder permission and define file location
		if(FA::getFilesystemHelper()->wp_mkdir_p($upload_dir['basedir'] . $this->templates_path)){
			$file = $upload_dir['basedir'] . $this->templates_path . '/' . $uid.'.zip';
	...
	public function _get_template_list($force = false){
		$rslb	= RevSliderGlobals::instance()->get('RevSliderLoadBalancer');
		$last_check	= FA::get_option('revslider-templates-check');

		if($last_check == false){ //first time called
			$last_check = 172801;
			FA::update_option('revslider-templates-check', time());
		}
		...
		if(time() - $last_check > 345600 || $force == true){ //4 days
            ...
            $hash = ($force === true) ? '' : FA::get_option('revslider-templates-hash', '');
			$code = (FA::get_option('revslider-valid', 'false') == 'false') ? '' : FA::get_option('revslider-code', '');
			$data	= array(
				'code'		=> urlencode($code),
				'shop_version' => urlencode(self::SHOP_VERSION),
	...
	private function update_template_list(){
		...
		FA::update_option('rs-templates-counter', $counter, false);
	}
	...
	public function copy_slide_to_templates($slide_id, $slide_title, $slide_settings = array()){
		...
		$slide_title = FA::sanitize_text_field($slide_title);
	...
	private function get_default_template_slides(){
		$templates = array();
		$templates = FA::apply_filters('revslider_set_template_slides', $templates);
	...
	public function get_default_template_sliders(){
        ...
        $sliders = FA::apply_filters('revslider_set_template_sliders', array());
	...
	public function get_tp_template_sliders($uid = false){
		...
		$latestVersion = FA::get_option('revslider-latest-version', Framework::RS_REVISION);
		$availableVersion = version_compare($latestVersion, Framework::RS_REVISION, '>') ? $latestVersion : Framework::RS_REVISION;
        $availablePlugins = array();
		foreach ((array) FA::get_option('revslider-addons', array()) as $pluginKey => $plugin) {
				...
				if(!empty($defaults[$dk]['plugin_require'])){
					foreach($defaults[$dk]['plugin_require'] as $pr => $plugin){
						...
						if(!isset($plugin_list[$path])){
							$plugin_list[$path] = (FA::is_plugin_active(FA::esc_attr($path))) ? true : false;
						}
	...
	public function get_tp_template_sliders_for_library($leave_counter = false){
		...
		if(!$this->_truefalse($leave_counter)){
			FA::update_option('rs-templates-counter', 0, false); //reset the counter
		}
		return $templates;
	}
