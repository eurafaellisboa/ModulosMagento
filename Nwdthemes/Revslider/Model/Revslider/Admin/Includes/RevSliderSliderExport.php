...
class RevSliderSliderExport extends RevSliderSlider {
	...
	public function __construct($title = 'export'){
		...
		$wp_upload_dir			= FA::wp_upload_dir();
	...
	public function export_slider($id = 0){
		...
		if(version_compare($this->get_param(array('settings', 'version')), FA::get_option('revslider_update_version', '6.0.0'), '<')){
	...
	public function modify_used_data(){
		...
		$d = FA::apply_filters('revslider_exportSlider_usedMedia', $d, $this->all_slides, $this->slider_params); //$this->export_slides, $this->static_slide,
	...
	public function serialize_export_data(){
		...
		$data = FA::apply_filters('revslider_exportSlider_export_data', $data, $this);
	...
	public function get_custom_animation_by_id($id){
		...
		if(empty(Framework::$revslider_animations)) return false;

		foreach(Framework::$revslider_animations as $animation){
			if($animation['id'] == $id){
				return array(
					'id'	 => $animation['id'],
	...
	public function add_svg_to_zip(){
		...
		$c_url	= str_replace(array('http:', 'https:'), '', FA::content_url());
	...
	public function add_info_to_zip(){
		if(FA::apply_filters('revslider_slider_pack_export', false)){
			if(!$this->usepcl){
