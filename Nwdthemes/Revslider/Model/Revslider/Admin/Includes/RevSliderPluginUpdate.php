...
class RevSliderPluginUpdate extends RevSliderFunctions {
	...
	public function __construct() {
		$this->revision = FA::get_option('revslider_update_version', '6.0.0');
	...
	public function get_version(){
		$real_version = FA::get_option('revslider_update_version', '5.2.6');
	...
	public function set_version($set_to){
		...
		FA::update_option('revslider_update_version', $set_to);
	...
	public function slider_need_update_checks(){
		$finished = FA::get_option('revslider_update_revision_current', '1.0.0');
	...
	public function upgrade_next_slider(){
		...
		FA::update_option('revslider_update_revision_current', $this->revision);
	...
	public function check_settings_table(){
		...
		if($wpdb->get_var("SHOW TABLES LIKE '" . $wpdb->prefix . RevSliderFront::TABLE_SETTINGS . "'") == $wpdb->prefix . RevSliderFront::TABLE_SETTINGS){
			...
			if(isset($result['general'])){
				FA::update_option('revslider-global-settings', $result['general']);
			}
		}
	}
	...
	public function add_general_settings($sliders = false){
		...
		if(!empty($sliders) && is_array($sliders)){
			$fonts = FA::get_option('tp-google-fonts', array());
				...
				if(!empty($slides) && is_array($slides)){
					foreach($slides as $slide){
						...
						if($ml_id !== false && $ml_id !== $image_id){
							$urlImage = FA::getImagesHelper()->wp_get_attachment_image_src($ml_id, 'full');
				...
				if($this->get_val($layer, 'scaleX', false) !== false){
					...
					if($get_dim === true){
						...
						if($cur_img !== ''){
							...
							$img_data = FA::wp_get_attachment_metadata($cur_id);
	...
	private function change_global_addon_settings_to_6_0(){
			...
			// 404 pages
			if(FA::is_plugin_active('revslider-404-addon/revslider-404-addon.php')){

				$settings = FA::get_option('revslider_404_addon');
				if($settings !== false){
					$options = array();
					parse_str($settings, $options);
					if(isset($options['revslider-404-addon-active']) && intval($options['revslider-404-addon-active'])){
						FA::update_option('revslider_404_enabled', 1);
					}
				}
			...
			if(FA::is_plugin_active('revslider-maintenance-addon/revslider-maintenance-addon.php')){

				$settings = FA::get_option('revslider_maintenance_addon');
				if($settings !== false){
					$options = array();
					parse_str($settings, $options);
					if(isset($options['revslider-maintenance-addon-active']) && intval($options['revslider-maintenance-addon-active'])){
						FA::update_option('revslider_maintenance_enabled', 1);
					}
				}
			...
			if(FA::is_plugin_active('revslider-backup-addon/revslider-backup-addon.php')){
				FA::update_option('revslider_backup_enabled', 1);
			}

			// featured slider
			if(FA::is_plugin_active('revslider-featured-addon/revslider-featured-addon.php')){
				FA::update_option('revslider_featured_enabled', 1);
			}

			// wp gallery
			if(FA::is_plugin_active('revslider-gallery-addon/revslider-gallery-addon.php')){
				FA::update_option('revslider_gallery_enabled', 1);
			}

			// login
			if(FA::is_plugin_active('revslider-login-addon/revslider-login-addon.php')){
				FA::update_option('revslider_login_enabled', 1);
			}

			// social sharing
			if(FA::is_plugin_active('revslider-sharing-addon/revslider-sharing-addon')){
				FA::update_option('revslider_sharing_enabled', 1);
			}

			// related posts slider
			if(FA::is_plugin_active('revslider-rel-posts-addon/revslider-rel-posts-addon.php')){
				FA::update_option('revslider_rel_posts_enabled', 1);
			}

			// prev/next slider
			if(FA::is_plugin_active('revslider-prevnext-posts-addon/revslider-prevnext-posts-addon.php')){
				FA::update_option('revslider_prevnext_posts_enabled', 1);
			}
	...
	public function change_global_settings_to_6_2_0(){
		$global = FA::maybe_unserialize(FA::get_option('revslider-global-settings', '')); //get the old structure as serialized
