...
class RevSliderSliderImport extends RevSliderSlider {
	...
	public function import_slider($update_animation = true, $exact_filepath = false, $is_template = false, $single_slide = false, $update_navigation = true, $install = true){
		...
		FA::do_action('revslider_slider_imported', $this->real_slider_id);
	...
	private function unzip_slider($exact_filepath = false){
		...
		$file = FA::getFilesystemHelper()->unzip_file($path, $this->download_path);

		if(FA::is_wp_error($file)){
		...
		if(!FA::is_wp_error($unzipped_data)){
			...
			foreach($navigations as $navigation){
				...
				if(!empty($_navigations)){
                    foreach($_navigations as $_navigation){
                        $exist = $wpdb->get_row($wpdb->prepare("SELECT id FROM ".$wpdb->prefix . RevSliderFront::TABLE_NAVIGATIONS." WHERE handle = %s AND type = %s", array($this->get_val($_navigation, 'handle'), $this->get_val($_navigation, 'type'))), Query::ARRAY_A);
	...
	public function process_slider_raw_data_post_6(){
		...
		$d = FA::apply_filters('revslider_importSliderFromPost_modify_slider_data', $d, $this->download_path, $this);
	...
	public function process_slide_data_pre_6(){
        ...
        foreach($this->slides_data as $slide_key => $slide){
					...
					if($this->import_zip === true){ //we have a zip, check if exists
						...
						if(isset($layer['type']) && $layer['type'] == 'svg'){
							if(isset($layer['svg']) && isset($layer['svg']->src)){
								$layer['svg']->src = FA::content_url().$layer['svg']->src;
							}
						}
					}
			...
			$d = FA::apply_filters('revslider_importSliderFromPost_modify_data', $d, 'normal', $this->download_path, $this);
				...
				foreach($layers as $layer_key => $layer){
					...
					if($this->import_zip === true){ //we have a zip, check if exists
						...
						if($layer_type == 'svg'){
							...
							$svgurl = ($zimage === true) ? $this->get_image_url_from_path($this->check_file_in_zip($this->download_path, $svg, $alias, $this->imported, true)) : FA::content_url().$svg;
	...
	public function process_static_slide_data_pre_6(){
		...
		if(!empty($static_slide) && $this->import_statics){
			foreach($static_slide as $slide){
				...
				$d = FA::apply_filters('revslider_importSliderFromPost_modify_data', $d, 'static', $this->download_path, $this);
				...
				if(!empty($layers)){
					foreach($layers as $layer_key => $layer){
						...
						if($type == 'svg'){
							...
							if(!$zimage) $zimage = $wp_filesystem->exists(str_replace('//', '/', $this->download_path.'images/'.$svg));
							$svgurl = ($zimage === true) ? $this->get_image_url_from_path($this->check_file_in_zip($this->download_path, $svg, $this->alias, $this->imported, true)) : FA::content_url().$svg;
							if(!empty($svg)) $layer['svg']['source'] = $svgurl;
						}
