...
class Attribute extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function getCount($filter, $stateFilters)
    {
        ...
        $conditions = [
            "{$tableAlias}.source_id = sub_products_stock_index.product_id",
            $connection->quoteInto("sub_products_stock_index.website_id = ?", 0),
            $connection->quoteInto("sub_products_stock_index.stock_id = ?", 1)
        ];

        $select->join(
           ['sub_products_stock_index' => 'cataloginventory_stock_status'],
            join(' AND ', $conditions),
            []
        );

        $select->where('sub_products_stock_index.stock_status = 1');
