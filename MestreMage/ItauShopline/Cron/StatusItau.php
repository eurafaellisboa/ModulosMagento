<?php
namespace MestreMage\ItauShopline\Cron;
        ...
        foreach($orderDatamodel as $orderDatamodel1) {
            if($order_number_itau_shopline = $orderDatamodel1->getPayment()->getAdditionalInformation('order_number_itau_shopline')){
               if($orderDatamodel1->getData('status') == 'pending'){
                   $itaucripto = new \MestreMage\ItauShopline\Api\Itaucripto();
                   $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORES;
                   $scopeConfig = $objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface');
                   $dc = $itaucripto->geraConsulta(strtoupper($scopeConfig->getValue('payment/itaushopline/company_code', $storeScope)), $order_number_itau_shopline, 01, $scopeConfig->getValue('payment/itaushopline/encrytion_Key', $storeScope));
                   ...
                   if($situacao_pag == '00'){
                       ...
                       if($scopeConfig->getValue('payment/itaushopline/order_invoice', $storeScope)){
